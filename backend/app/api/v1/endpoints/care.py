"""
Vaidya — /api/v1/care  (Module 7 — complete)

GET  /hospitals              — ranked hospitals + PHCs via OSM + ABDM
...
GET  /coverage               — PMJAY / state scheme eligibility via ABDM
...
@router.get(
    "/coverage",
    summary="Check PMJAY / state scheme eligibility via ABDM",
)
async def check_coverage(
    aadhaar_token: str = Query(
        ...,
        min_length=10,
        description="OTP-verified ABDM transaction token (not raw Aadhaar)",
    ),
    state_code: Optional[str] = Query(
        None,
        max_length=3,
        description="ISO 3166-2:IN state code e.g. TN, MH, AP",
    ),
):
    """
    Verifies PMJAY and state insurance scheme eligibility using a tokenised
    Aadhaar transaction token from ABDM.

    **IMPORTANT:** This endpoint must receive the ABDM OTP-verified token,
    NOT the raw 12-digit Aadhaar number. Raw Aadhaar must never pass through
    Vaidya's systems (DPDP Act 2023 compliance).

    Returns: scheme name, annual cover, state scheme, nearest empanelled hospitals.
    """
    return await check_pmjay_coverage(
